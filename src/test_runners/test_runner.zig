...
const SlowTracker = struct {
    ...
    fn startTiming(self: *SlowTracker) void {
        self.timer = .now(self.io, .awake);
    }

    fn endTiming(self: *SlowTracker, test_name: []const u8) u64 {
        const ns: u64 = @intCast(self.timer.untilNow(self.io, .awake).toNanoseconds());
...
const Env = struct {
    ...
    fn init(allocator: Allocator, environ: *const std.process.Environ.Map) Env {
        return .{
    ...
    fn readEnv(allocator: Allocator, environ: *const std.process.Environ.Map, key: []const u8) ?[]const u8 {
        const value = environ.get(key) orelse return null;
        return allocator.dupe(u8, value) catch @panic("OOM");
    }

    fn readEnvBool(allocator: Allocator, environ: *const std.process.Environ.Map, key: []const u8, deflt: bool) bool {
